...
if [[ $# -lt 1 || $# -gt 3 ]]; then
  ...
  echo "Example: $0 metadata/sample_chunks_20/chunk_00 -Xmx4g"
  echo "Example: $0 metadata/sample_chunks_20/chunk_00 -Xmx4g --disable-pon"
  exit 1
fi
...
SCRIPT="${PROJECT_DIR}/scripts/mutect2_one_sample.py"
LOG_DIR="${PROJECT_DIR}/results/logs"

mkdir -p "${LOG_DIR}"

CHUNK_NAME="$(basename "${SAMPLE_LIST}")"
MASTER_LOG="${LOG_DIR}/${CHUNK_NAME}.mutect2.master.log"
DONE_TSV="${LOG_DIR}/${CHUNK_NAME}.mutect2.done.tsv"
FAIL_TSV="${LOG_DIR}/${CHUNK_NAME}.mutect2.failed.tsv"
